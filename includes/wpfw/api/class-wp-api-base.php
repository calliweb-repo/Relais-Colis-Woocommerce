...
abstract class WP_API_Base {
    ...
    /** @var WP_API_Request|object request */
    protected $request;
    ...
    /** @var WP_API_Response|object response */
    protected $response;
     ...
     * @since 1.0.0
     *
     * @param WP_API_Request|object $request class instance which implements WP_API_Request
     * @return WP_API_Response|object class instance which implements WP_API_Response
     * @throws Exception may be thrown in implementations
     */
     ...
     * @return WP_API_Request|object request class instance that implements WP_API_Request
     */
    protected function handle_response( $response ) {
        ...
        // Qui est le con qui reference une classe nimp depuis ici ? 
        // $this->response = $this->get_parsed_response( $this->raw_response_body );
        $this->response = $this->raw_response_body ;
     ...
     * processing should continue, or throw a \Exception with a
     ...
     * @return object|WP_API_Request response class instance which implements WP_API_Request
...
//        if ( $request && in_array( strtoupper( $this->get_request_method() ), array( 'GET', 'HEAD' ), true ) ) {
    ...
    protected function get_request_body() {
        ...
        // Set body with data array from request
        return $this->get_request()->get_data();
...
//        return sprintf( '%s/%s (WooCommerce/%s; WordPress/%s)', str_replace( ' ', '-', WP_PLoad::pluginfo(WP_PLoad::TEXT_DOMAIN) ), WP_PLoad::pluginfo(WP_PLoad::VERSION), WC_VERSION, $GLOBALS['wp_version'] );
    }
     ...
     * @since 1.0.0
     *
     * @return WP_API_Request|object the most recent request object
     */
     ...
     * @since 1.0.0
     *
     * @return WP_API_Response|object the most recent response object
     */
     ...
     *
     * Note the class should implement WP_API
     *
     * @since 1.0.0
     *
    ...
    public function set_tls_1_2_request( $handle, $r, $url ) {
        ...
        if ( ! WP_Helper::str_starts_with( $url, 'https://' ) ) {
            return;
        }
