...
class WC_RC_Ajax_Get_Wc_Products {
    ...
    public function get_wc_products() {
        ...
        WP_Log::debug( __METHOD__, [ 'GET'=>$_GET ], 'relais-colis-officiel');

        $nonce_check = check_ajax_referer( 'rc_multiselect_products_nonce', 'nonce', false );
        if ( !$nonce_check ) {

            WP_Log::error( __METHOD__.' - Nonce verification failed', [ 'received_nonce' => $_GET['nonce'] ?? 'MISSING' ], 'relais-colis-officiel');
